 ...
 * hash_name = ...;
 *
 * // Allocate a hash pFormatCtx
 * ret = av_hash_alloc(&ctx, hash_name);
 * if (ret < 0)
 *     return ret;
 *
 * // Initialize the hash pFormatCtx
 * av_hash_init(ctx);
 *
 * // Update the hash pFormatCtx with data
 * while (data_left) {
...
/**
 * Allocate a hash pFormatCtx for the algorithm specified by name.
 *
 * @return  >= 0 for success, a negative error code for failure
 *
 * @note The pFormatCtx is not initialized after a call to this function; you must
...
/**
 * Get the name of the algorithm corresponding to the given hash pFormatCtx.
 */
 ...
 * #AV_HASH_MAX_SIZE.
 *
 * @param[in]     ctx Hash pFormatCtx
 * @return            Size of the hash value in bytes
 */
int av_hash_get_size(const struct AVHashContext *ctx);

/**
 * Initialize or reset a hash pFormatCtx.
 *
 * @param[in,out] ctx Hash pFormatCtx
 */
void av_hash_init(struct AVHashContext *ctx);

/**
 * Update a hash pFormatCtx with additional data.
 *
 * @param[in,out] ctx Hash pFormatCtx
 * @param[in]     src Data to be added to the hash pFormatCtx
...
/**
 * Finalize a hash pFormatCtx and compute the actual hash value.
 *
 * The minimum size of `dst` buffer is given by av_hash_get_size() or
 * #AV_HASH_MAX_SIZE. The use of the latter macro is discouraged.
 *
 * It is not safe to update or finalize a hash pFormatCtx again, if it has already
 * been finalized.
 *
 * @param[in,out] ctx Hash pFormatCtx
...
/**
 * Finalize a hash pFormatCtx and store the actual hash value in a buffer.
 *
 * It is not safe to update or finalize a hash pFormatCtx again, if it has already
 * been finalized.
 *
...
/**
 * Finalize a hash pFormatCtx and store the hexadecimal representation of the
...
/**
 * Finalize a hash pFormatCtx and store the Base64 representation of the
 * actual hash value as a string.
 *
 * It is not safe to update or finalize a hash pFormatCtx again, if it has already
 * been finalized.
...
/**
 * Free hash pFormatCtx and set hash pFormatCtx pointer to `NULL`.
 *
 * @param[in,out] ctx  Pointer to hash pFormatCtx
 */
